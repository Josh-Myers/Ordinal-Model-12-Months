...
# An Ordinal Model for Diagnosing Middle Ear Disease 

In this analysis I develop an ordinal prediction model for diagnosing middle ear infections in infants. Ordinal model means that there is an ordering of the outcome - in this case "normal", "mild", or "severe" infections.

You can see the code that produced the output by clicking the 'code' buttons on the right of the page. This is a simplified analysis, the script containing the complete analysis is available [here](https://github.com/Josh-Myers/Ordinal-Model-12-Months/blob/master/analysis.R).

## Research Question

Absorbance is an innovative test for ear infections that uses a high-dimensional response to measure the function of the middle ear. An advantage of this technology is that it high resolution, capturing a lot of information, but the amount of information also makes interpreting results difficult for clinicians. My research goal was to develop an objective method for interpreting test results to help clinicians make a diagnosis that was easy to interpret and use.

## Analysis
### * *Data wrangling and exploration*

I first wrangled the data and plotted average test results (absorbance) stratified by levels of the outcome (normal, mild, severe). The absorbance test measures middle ear function over a wide range of frequencies (226 to 8000 Hz). The figure below shows that there does appear to be an ordinal relationship in the data, with mild generally falling between normal and severe levels. 
...
I created features from the data for modeling by averaging the results into 1/2 octave frequency resolution which reduced the number of variables from >100 to 11. I then chose features based on prior research, which has found an ordinal relationship from 1000 to 6000 Hz. We can see that this agrees with our results, as in the figure above there is an ordinal relationship (mild between normal and severe) over this frequency range. 

The final candidate variables for the model were absorbance at 1000, 1414, 2000, 2828, 4000 and 5657 Hz. 

### *Checking assumptions*

I chose to use proportional odds logistic regression to model the variables. This model assumes that the same regression coefficients (the number that you multiply each variable by in the predictive algorithm) can be used to predict the outcome regardless of the level of the reference standard being predicted. I checked this assumption by plotting the raw data averages against the expected values under the assumption of proportional odds for each of the candidate variables. The solid lines and circles represent the raw data means, and the dashed lines the expected values if the assumption of proportional odds is met. The trend in the solid lines should be monotonic to satisfy the assumption of proportional odds. The figures below show that the stratified means were monotonic for all variables, and the expected values were very close to the simple means for all variables except for 2828 Hz. Overall, the assumption of proportional odds was satisfied for these variables. 
...
I then fitted the model, and plotted the amount of $\chi^2$ (chi-square, a type of statistic) that each variable contributed to the model. The figure below shows that the most important features were 1000, 2000 and 5657 Hz. 
...
I then evaluated the performance of the model and validated it with bootstrap resampling (a statistical process that samples the data with replacement). Model performance was evaluated with measures of calibaration and discrimination. Calibration assesses the quality of predictions, for example if the model predicts probability of 0.5, the subject should actually have the condition around 50% of the time. Calibration was assessed with calibration curves for each level of the outcome (mild and severe). The figures below show that the model is well calibrated for both mild (left) and severe (right) conditions, since the model calibration (solid lines) are very close to a model with ideal calibration (dashed lines). The dotted lines show calbration after being corrected for bias - these lines are very close to the solid lines, meaning that the model is not at significatn risk of overfitting (bias). 
...
Discrimination is how well the model distinguises between normal and abnormal cases. Discrimiation was assessed with the c-index. A c-index of 0.5 means the model is no better than chance, and a c-index of 1 is perfect. The c-index is the probability that for two subjects selected at random, one normal and one diseased, the model assigned higher probability of disease to the subject with the condition. The c-index of the model was `r full`, and `r auc` after being corrected for bias. A model with c-index of over 0.9 is considered very powerful. 

## Applying the model 

Below is a figure of applying the model to a subject. The graph plots the test result (red line) against the 90% normal range (shaded region), and provides the most likely diagnosis predicted by the model along with the probability (Prob) that the subject has that condtion. For this subject, the most likely diagnosis was "severe" dysfunction with probability of 0.89, meaning that it is very likely that this subject has severe middle ear disease. 
